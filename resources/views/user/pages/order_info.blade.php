...
<ul>
    <li>გაქირავება {{ Carbon\Carbon::parse($order->date_from)->diffInDays($order->date_to) }} დღით <span>{{ $order->price }} $</span></li>
...
<p>დეპოზიტი 100$</p>
@endif
<h6>სულ შემოსავალი {{ $order->total_price }}$</h6>
            ...
            directionsService.route(request, function(result, status) {
                if (status == 'OK') {
					...
					var startMarker = new google.maps.Marker({
                            position: start,
                            map: map,
                            icon: 'https://tripbox.ge/assets/images/svg/location_vector.svg', // Optional: customize the icon
                            title: 'Start'
                        });
